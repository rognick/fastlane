module Fastlane
  module Actions
    module SharedValues
      ...
      FL_JIRA_ISSUE_IDS_URL = :FL_JIRA_ISSUE_IDS_URL
    end
    ...
    class JiraIssueKeysFromChangelogAction < Action
      def self.run(params)
        ...
        p = Array.new
        puts "Issues found: #{issue_keys.count}"

        issue_keys.each {|item| p << "[#{item}](#{options[:jira_url]}#{item})"}

        puts p

        Actions.lane_context[SharedValues::FL_JIRA_ISSUE_IDS] = issue_keys
        Actions.lane_context[SharedValues::FL_JIRA_ISSUE_IDS_URL] = p
        return issue_keys
      end
